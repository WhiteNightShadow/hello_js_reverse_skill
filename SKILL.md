...
```
readFile("cases/<命中的 case>.md")  # 读取方案方向 + 踩坑记录

案例提供每个 Phase 的加速指引，不替代 Phase：
  Phase 1: 仍然执行侦察，用 scripts(action='save') 保存 SDK 脚本到本地 config/
  Phase 2: 仍然执行源码分析
  Phase 3: 仍然执行动态验证（确认需要补哪些环境项后再写补丁）
  Phase 4: 参考案例踩坑记录编码，环境补丁逐项添加逐项验证
  Phase 5: 验证

⚠️ 命中案例 = 知道终点在哪，不等于跳过路上的步骤
```
...
11. **命中案例不等于跳过分析流程**：案例提供方案方向和踩坑经验，Phase 1-5 仍然正常走，SDK 脚本仍要保存到本地，环境补丁仍要逐项验证后再添加
